...
import { IEmailLocals, winstonLogger } from '@Zephyr-Jeon/jobber-shared';
...
async function consumeAuthEmailMessages(channel: Channel): Promise<void> {
  try {
    ...
    channel.consume(jobberQueue.queue, async (msg: ConsumeMessage | null) => {
      console.log(2727, JSON.parse(msg?.content.toString() || 'test'));
...
async function consumeOrderEmailMessages(channel: Channel): Promise<void> {
  try {
    ...
    channel.consume(jobberQueue.queue, async (msg: ConsumeMessage | null) => {
      console.log(9090, JSON.parse(msg?.content.toString() || 'test'));
